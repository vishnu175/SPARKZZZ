...
@borg.on(sudo_cmd(outgoing=True, pattern="kang", allow_sudo=True))
async def kang(args):
    ...
    if photo:
        ...
        packname = f"a{user.id}_by_SP_{pack}"
        packnick = f"@{user.username}'s SPARKZZZ Vol.{pack}"
        cmd = '/newpack'
        file = io.BytesIO()
...
@borg.on(sudo_cmd(outgoing=True, pattern="stkrinfo", allow_sudo=True))
