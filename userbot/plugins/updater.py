...
# Rewritten for SPARKZZZ by @vishnu175

"""
This module is used for updating SPARKZZZ
"""
...
async def gen_chlog(repo, diff):
    ...
    for c in repo.iter_commits(diff):
        ch_log += f"🔨 **#{c.count()}** : {c.committed_datetime.strftime(d_form)} : [{c.summary}]({UPSTREAM_REPO_URL.rstrip('/')}/commit/{c}) by 🕵️ **{c.author}**\n"
    return ch_log
...
@sparkzzz.on(admin_cmd(pattern="update ?(.*)"))
